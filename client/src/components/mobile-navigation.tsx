...
import { FileEdit, FileText, Briefcase } from 'lucide-react';
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-t shadow-lg md:hidden z-50">
      <div className="container flex justify-around py-3">
        <Link href="/resume-builder" className="flex flex-col items-center p-2 text-muted-foreground hover:text-foreground">
          <FileText className="h-5 w-5" />
          <span className="text-xs mt-1">Create</span>
        </Link>
        <Link href="/resume-editor" className="flex flex-col items-center p-2 text-muted-foreground hover:text-foreground">
