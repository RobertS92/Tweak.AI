...
export default function ResumeBuilder() {
  ...
  // AI Assistant states
  const [assistantQuery, setAssistantQuery] = useState(""); // user query
  ...
  /**
   * 3. The AI Assistant function to get suggestions for the active section
   *    (calls /api/resume-ai-assistant)
   */
  const handleAiAssistantQuery = async () => {
    if (!activeSection) {
      toast({
        title: "No Section Selected",
        description: "Please select a resume section first.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (!currentSection) {
      toast({
        ...
        description: "The selected section doesn't exist.",
        variant: "destructive",
      });
      return;
    }

    // Build a string from the current section's content or items
    let sectionContent = "";
    if (currentSection.items) {
      // Combine item data for the AI
      sectionContent = currentSection.items
        .map((item) => {
    ...
    try {
      setIsAnalyzing(true);
      setAiMessage("Analyzing the selected section...");
      ...
      const data = await response.json();
      // data.suggestions is the text from GPT
      setAiMessage(data.suggestions || "No suggestions found.");
    } catch (err) {
      ...
      setAiMessage("Unable to get suggestions. Please try again.");
    } finally {
      setIsAnalyzing(false);
    }
  };
        ...
        <Card className="mt-6">
          ...
          <CardContent className="p-0">
            ...
            <div className="p-4 border-t flex gap-2">
              ...
              />
              <Button size="icon" onClick={handleAiAssistantQuery}>
